...
JT_VERSION=$(cat JT_VERSION | tr -d '[:space:]')
JT_HOME="${HOME}/.tt"
ENV_VAR_FILE="${JT_HOME}/env/local.env"
DATA_DIR="${JT_HOME}"
EXTERNAL_PORT=7777
BACKGROUND_FLAGS=""
...
if ! docker image inspect "jt:${JT_VERSION}" > /dev/null 2>&1; then
    echo "Error: Docker image 'jt' does not exist. Please build it first: 'make docker-build'"
    exit 1
fi

docker rm jt 2>/dev/null || true
