...
class EnvironmentConfig:
    ...
    def get( cls, env_name : str ):
        jt_home = os.path.join( os.environ['HOME'], '.jt' )
        if env_name == 'local':
            return EnvironmentConfig(
                label = env_name,
                runs_in_docker = True,
                data_directory = '/data',  # Location relative to internal Docker container
                secrets_directory = os.path.join( jt_home, 'env' ),
                secrets_suffix = 'env',
...
class JtEnvironmentGenerator:
    ...
    def __init__( self,
        ...
        if env_name not in [ 'development', 'local', 'staging', 'production' ]:
            self.print_warning( f'Non-standard environment name "{env_name}".'
                                f' Ensure that the file "jt/settings/{env_name}.py" exists.' )
        ...
        self._settings_map = {
            'DJANGO_SETTINGS_MODULE': f'jt.settings.{self._env_name}',
            'DJANGO_SERVER_PORT': self._env_config.django_server_port,
            'JT_SUPPRESS_AUTHENTICATION':'true',
            'JT_REDIS_HOST': '127.0.0.1',
            'JT_REDIS_PORT': '6379',
            'JT_REDIS_KEY_PREFIX': self._env_config.redis_key_prefix,
            'JT_EMAIL_SUBJECT_PREFIX': self._env_config.redis_subject_prefix,
            'JT_EXTRA_HOST_URLS': '',  # To be filled in manually if/when running beyond localhost
            'JT_EXTRA_CSP_URLS': '',  # To be filled in manually if/when running beyond localhost
        }
    ...
    def generate_env_file( self ):
        ...
        self._settings_map['JT_DB_PATH'] = db_dir
        self._settings_map['JT_MEDIA_PATH'] = media_dir
        ...
        if email_settings.is_valid:
            ...
            if require_signin:
                self._settings_map['JT_SUPPRESS_AUTHENTICATION'] = 'false'
        ...
        self._settings_map['JT_DEFAULT_FROM_EMAIL'] = from_email
        self._settings_map['JT_SERVER_EMAIL'] = server_email
        self._settings_map['JT_EMAIL_HOST_USER'] = email_settings.email_address
        self._settings_map['JT_EMAIL_HOST_PASSWORD'] = email_settings.password
        self._settings_map['JT_EMAIL_HOST'] = email_settings.smtp_settings.host
        self._settings_map['JT_EMAIL_PORT'] = str(email_settings.smtp_settings.port)
        self._settings_map['JT_EMAIL_USE_TLS'] = str(email_settings.smtp_settings.use_tls)
        self._settings_map['JT_EMAIL_USE_SSL'] = str(email_settings.smtp_settings.use_ssl)
...
def parse_command_line_args():
    ...
    parser = argparse.ArgumentParser(
        description = 'Generate environment variables for Journal Tools.',
        add_help = True,
    )
    parser.add_argument(
        '--env-name',
...
if __name__ == '__main__':
    ...
    generator = JtEnvironmentGenerator(
