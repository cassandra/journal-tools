...
# Journal Tools Development Setup Script
# 
...
print_important "Journal Tools Development Setup
...
if [[ ! -f "JT_VERSION" || ! -d "deploy" || ! -d "docs" ]]; then
    print_error "This script must be run from the project root directory."
    print_notice "Please cd to the journal-tools directory and try again."
    exit 1
fi
...
if [[ -z "$CURRENT_ORIGIN" ]]; then
    ...
    if prompt_yes_no "Do you have SSH keys set up with GitHub?" "n"; then
        git remote add origin "git@github.com:${GITHUB_USERNAME}/journal-tools.git"
    else
        git remote add origin "https://github.com/${GITHUB_USERNAME}/journal-tools.git"
    fi
...
pip install --quiet -r src/jt/requirements/development.txt
