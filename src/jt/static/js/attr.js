/*
 * Journal Tools - Attribute Editing JavaScript
...
(function() {
    ...
    // Create the main Jt.attr namespace
    window.Jt = window.Jt || {};
    ...
    const _ajax = {
        ...
        submitFormWithAjax: function(form, options = {}) {
            ...
            const $container = $form.closest(Jt.ATTR_V2_CONTAINER_SELECTOR);
            if ($container.length > 0) {
        ...
        handleResponse: function(response, $form, options = {}, isError = false) {
            ...
            if (data.redirect) {
                if ( Jt.DEBUG ) { console.log('Form response contains redirect, navigating to:', data.redirect); }
        ...
        showStatusMessage: function(message, type = STATUS_TYPE.INFO, $element = null, timeout = null) {
            ...
            const $container = $element ? $($element).closest(Jt.ATTR_V2_CONTAINER_SELECTOR) : $(Jt.ATTR_V2_CONTAINER_SELECTOR).first();
            const $statusMsg = $container.find(Jt.ATTR_V2_STATUS_MESSAGE_SELECTOR);
            ...
            if (timeout > 0) {
                setTimeout(() => {
                    $statusMsg.text('')
                             .attr('class', `${Jt.ATTR_V2_STATUS_MESSAGE_CLASS} ml-2`);  // Reset to base class
                }, timeout);
            }
        }
    };
    ...
    function _updateFormAction(newUrl, containerId, labelText = null) {
        if (!newUrl || !containerId) {
            console.warn('Jt.attr.updateFormAction: newUrl and containerId are required');
            return;
        }
        ...
        const $container = $(`#${containerId}`);
        if ($container.length === 0) {
            console.warn('Jt.attr.updateFormAction: Container not found:', containerId);
            return;
        }

        const $form = $container.find(Jt.ATTR_V2_FORM_CLASS_SELECTOR);
        if ($form.length === 0) {
            console.warn('Jt.attr.updateFormAction: No form found in container:', containerId);
            return;
        }
        ...
        if (labelText) {
            const $label = $container.find(Jt.ATTR_V2_FORM_DISPLAY_LABEL_SELECTOR);
            if ($label.length > 0) {
    ...
    function _initializeAllContainers() {
        ...
        $(Jt.ATTR_V2_CONTAINER_SELECTOR).each(function() {
    ...
    function _initializeContainer($container) {
        ...
        if ($container.data(ATTR_V2_INTERNAL.INITIALIZED_DATA_KEY)) {
            ...
            if (window.Jt.attr.dirtyTracking) {
                window.Jt.attr.dirtyTracking.reinitializeContainer($container.attr('id'));
            }
            return;
        }
        ...
        if (window.Jt && window.Jt.attr && window.Jt.attr.dirtyTracking) {
            window.Jt.attr.dirtyTracking.reinitializeContainer($container.attr('id'));
        }
    ...
    function _handleAutoDismissMessages($container) {
        const $statusMsg = $container.find(Jt.ATTR_V2_STATUS_MESSAGE_SELECTOR);
        const $dismissibleElements = $statusMsg.find(Jt.ATTR_V2_AUTO_DISMISS_SELECTOR);
    ...
    function _setupCustomAjaxHandlers($container) {
        ...
        const $forms = $container.find(`form${Jt.ATTR_V2_FORM_CLASS_SELECTOR}`);
        ...
        const $historyLinks = $container.find(Jt.ATTR_V2_HISTORY_LINK_SELECTOR);
        $historyLinks.each(function() {
            ...
            $link.on(`click.${ATTR_V2_INTERNAL.AJAX_EVENT_NAMESPACE}`, function(e) {
                ...
                $.ajax({
                    ...
                    _ajax.handleFormSuccess(response, $container.find(Jt.ATTR_V2_FORM_CLASS_SELECTOR));
                }).fail((xhr) => {
                    _ajax.handleFormError(xhr, $container.find(Jt.ATTR_V2_FORM_CLASS_SELECTOR));
                });
            });
        });
        
        // Handle value restore links  
        const $restoreLinks = $container.find(Jt.ATTR_V2_RESTORE_LINK_SELECTOR);
        ...
        const $fileInput = $container.find(Jt.ATTR_V2_FILE_INPUT_SELECTOR);
    ...
    function _showAddAttribute(containerSelector = null) {
        ...
        const attributeCards = scope.find(Jt.ATTR_V2_ATTRIBUTE_CARD_SELECTOR);
    ...
    window.markFileForDeletion = function(attributeId, containerSelector = null) {
        ...
        const scope = containerSelector ? $(containerSelector) : $(document);
        const $fileCard = scope.find(`${Jt.ATTR_V2_FILE_CARD_SELECTOR}[${Jt.DATA_ATTRIBUTE_ID_ATTR}="${attributeId}"]`);
        if ($fileCard.length === 0) return;
        
        // The display "name" for a file is its attribute.value, *not* attribute.name
        const fileValue = $fileCard.find(Jt.ATTR_V2_FILE_TITLE_INPUT_SELECTOR).val().trim();

        // Find and mark the server-rendered DELETE field for deletion
        const $deleteField = $fileCard.find(`input[name="${Jt.ATTR_V2_DELETE_FILE_ATTR}"]`);
        if ($deleteField.length > 0) {
            ...
            const $container = $fileCard.closest(Jt.ATTR_V2_CONTAINER_SELECTOR);
            if ($container.length > 0 && window.Jt.attr.dirtyTracking) {
                const containerId = $container.attr('id');
                const instance = window.Jt.attr.dirtyTracking.getInstance(containerId);
        ...
        $fileCard.find(Jt.ATTR_V2_DELETE_BTN_SELECTOR).hide();
        $fileCard.find(Jt.ATTR_V2_UNDO_BTN_SELECTOR).show();
    ...
    window.undoFileDeletion = function(attributeId, containerSelector = null) {
        ...
        $fileCard.find(Jt.ATTR_V2_DELETE_BTN_SELECTOR).show();
        $fileCard.find(Jt.ATTR_V2_UNDO_BTN_SELECTOR).hide();
    ...
    window.markAttributeForDeletion = function(attributeId, containerSelector = null) {
        ...
        const scope = containerSelector ? $(containerSelector) : $(document);
        const $attributeCard = scope.find(`[${Jt.DATA_ATTRIBUTE_ID_ATTR}="${attributeId}"]`);
        if ($attributeCard.length === 0) return;
        
        const attributeName = $attributeCard.find(Jt.ATTR_V2_ATTRIBUTE_NAME_SELECTOR).text().trim().replace('•', '').trim();
        ...
        // Hide delete button and show undo button (both server-rendered)
        $attributeCard.find(Jt.ATTR_V2_DELETE_BTN_SELECTOR).hide();
        $attributeCard.find(Jt.ATTR_V2_UNDO_BTN_SELECTOR).show();
    ...
    window.undoAttributeDeletion = function(attributeId, containerSelector = null) {
        ...
        // Show delete button and hide undo button (both server-rendered)
        $attributeCard.find(Jt.ATTR_V2_DELETE_BTN_SELECTOR).show();
        $attributeCard.find(Jt.ATTR_V2_UNDO_BTN_SELECTOR).hide();
    ...
    function _toggleSecretField(button) {
        const $button = $(button);
        const $input = $button.closest(Jt.ATTR_V2_SECRET_INPUT_WRAPPER_SELECTOR).find(Jt.ATTR_V2_SECRET_INPUT_SELECTOR);
        const $showIcon = $button.find(Jt.ATTR_V2_ICON_SHOW_SELECTOR);
        const $hideIcon = $button.find(Jt.ATTR_V2_ICON_HIDE_SELECTOR);
    ...
    function _updateBooleanHiddenField(checkbox) {
        const hiddenFieldId = checkbox.getAttribute(Jt.DATA_HIDDEN_FIELD_ATTR);
    ...
    function _initializeAutosizeTextareas() {
        ...
        const textareas = $(Jt.ATTR_V2_TEXTAREA_SELECTOR).not('.truncated');
    ...
    function _reinitializeTextareas($container = null) {
        ...
        const textareas = $container ? 
            $container.find(Jt.ATTR_V2_TEXTAREA_SELECTOR) : 
            $(Jt.ATTR_V2_TEXTAREA_SELECTOR);
        ...
        textareas.each(function() {
            const textarea = $(this);
            const wrapper = textarea.closest(Jt.ATTR_V2_TEXT_VALUE_WRAPPER_SELECTOR);
            const isOverflowing = wrapper.attr(Jt.DATA_OVERFLOW_ATTR) === 'true';
            
            // Check if this is a display field (new pattern) or legacy textarea
            const hiddenFieldId = textarea.attr(Jt.DATA_HIDDEN_FIELD_ATTR);
        ...
        const editableTextareas = $(Jt.ATTR_V2_TEXTAREA_SELECTOR).not('.truncated').not('[readonly]');
    ...
    function _updateOverflowState(textarea) {
        ...
        const wrapper = textarea.closest(Jt.ATTR_V2_TEXT_VALUE_WRAPPER_SELECTOR);
        wrapper.attr(Jt.DATA_OVERFLOW_ATTR, overflows ? 'true' : 'false');
        wrapper.attr(Jt.DATA_LINE_COUNT_ATTR, lineCount);
    ...
    function _applyTruncationFromHidden(displayField, hiddenField) {
        ...
        const wrapper = displayField.closest(Jt.ATTR_V2_TEXT_VALUE_WRAPPER_SELECTOR);
        const expandControls = wrapper.find(Jt.ATTR_V2_EXPAND_CONTROLS_SELECTOR);
        expandControls.show();
    ...
    function _initializeExpandableTextareas() {
        ...
        const displayTextareas = $(Jt.ATTR_V2_DISPLAY_FIELD_SELECTOR);
        
        displayTextareas.each(function() {
            const displayField = $(this);
            const isOverflowing = displayField.attr(Jt.DATA_OVERFLOW_ATTR) === 'true';
            const hiddenFieldId = displayField.attr(Jt.DATA_HIDDEN_FIELD_ATTR);
    ...
    function _toggleExpandedView(button) {
        const $button = $(button);
        const wrapper = $button.closest(Jt.ATTR_V2_TEXT_VALUE_WRAPPER_SELECTOR);
        const displayField = wrapper.find(Jt.ATTR_V2_DISPLAY_FIELD_SELECTOR
                                          + ', ' + Jt.ATTR_V2_TEXTAREA_SELECTOR); // Support both new and legacy
        const showMoreText = $button.find(Jt.ATTR_V2_SHOW_MORE_TEXT_SELECTOR);
        const showLessText = $button.find(Jt.ATTR_V2_SHOW_LESS_TEXT_SELECTOR);
        ...
        if (displayField.prop('readonly')) {
            ...
            if (!overflows) {
                ...
                displayField.prop('readonly', false);
                displayField.removeClass(ATTR_V2_INTERNAL.TRUNCATED_CLASS);
                wrapper.find(Jt.ATTR_V2_EXPAND_CONTROLS_SELECTOR).hide();
                
                // Update wrapper state
                wrapper.attr(Jt.DATA_OVERFLOW_ATTR, 'false');
    ...
    function _syncTextareaValuesToHiddenFields($container) {
        ...
        $container.find(Jt.ATTR_V2_DISPLAY_FIELD_SELECTOR).each(function() {
            const displayField = $(this);
            const hiddenFieldId = displayField.attr(Jt.DATA_HIDDEN_FIELD_ATTR);
