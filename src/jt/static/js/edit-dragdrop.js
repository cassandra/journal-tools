(function() {
    window.Jt = window.Jt || {};
    window.Jt.edit = window.Jt.edit || {};

    const JtEditDragDrop = {
        // Externally accessible functions here.
    };

    window.Jt.edit.path = JtEditDragDrop;
    ...
    */
    ...
    function handleDragStart( event ) {
        if ( Jt.DEBUG ) { console.log('event.currentTarget:', event.currentTarget); }
        ...
        setTimeout(() => {
            ...
            if ( Jt.DEBUG ) { console.log('Hidden class added'); }
        }, 0);
    }
    
    function handleDragEnd( event ) {
        if ( Jt.DEBUG ) { console.log('Drag end:'); }
        ...
        if ( Jt.DEBUG ) { console.log(`Drag end ids: ${htmlIdList}`); }
    ...
    function handleDragOver( event ) {
        if ( Jt.DEBUG ) { console.log('Drag over:'); }
    ...
    function handleDragEnter( event ) {
        if ( Jt.DEBUG ) { console.log('Drag enter:'); }
    ...
    function handleDragLeave( event ) {
        if ( Jt.DEBUG ) { console.log('Drag leave:'); }
