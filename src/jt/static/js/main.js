(function() {
    ...
    const Jt = {
        ...
        // JtClientConfig comes from server-side template injection. This module
        // (main.css) should be the only Javaacript that knows about
        // JtClientConfig and the coordination mechanism with those
        // server-delivered config settings.  All other JS modules should
        // look to this module to relay any needed config settings (via
        // variable below).
        //
        DEBUG: window.JtClientConfig?.DEBUG ?? false,
        isEditMode: window.JtClientConfig?.IS_EDIT_MODE ?? false,
    ...
    function _submitForm( formElement ) {
        ...
        if ( Jt.DEBUG ) { console.debug( 'Submitting form:', formElement, form ); }
        $(form).submit();
    }
    
    function _togglePasswordField( toggleCheckbox ) {

        let passwordField = $(toggleCheckbox).closest(Jt.FORM_FIELD_CONTAINER_SELECTOR).find('input[type="password"], input[type="text"]');
