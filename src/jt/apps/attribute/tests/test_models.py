...
from jt.apps.attribute.enums import AttributeValueType, AttributeType
...
class TestAttributeModel(BaseTestCase):
    ...
    @patch('jt.apps.attribute.models.PredefinedValueRanges.get_choices')
    def test_attribute_model_choices_predefined_lookup(self, mock_get_choices):
        ...
        attr = ConcreteAttributeModel(
            ...
            value_range_str='jt.test.choices'
        )
        
        choices = attr.choices()
        
        # Should use predefined choices, not parse JSON
        mock_get_choices.assert_called_once_with('jt.test.choices')
        self.assertEqual(choices, [('pred1', 'Predefined 1'), ('pred2', 'Predefined 2')])
        return

    @patch('jt.apps.attribute.models.generate_unique_filename')
    ...
    @patch('jt.apps.attribute.models.default_storage')
